...
#define	SATELLITE_ORBIT		 0.5	// Orbit Radius of the Pluto
...
static GLfloat satelliter_Angle;
static GLfloat satellites_Angle;

int operation = 0;				//operation of behavior. Default is 0.
GLboolean g = GL_TRUE;			//停止公转
GLboolean t = GL_TRUE;			//停止自转
GLboolean r = GL_FALSE;			//重置
...
void Satellite()
{
	...
	glBegin(GL_QUADS);
	glNormal3f(0.0f, 0.0f, 1.0f);
	glVertex3f(-0.01f, 0.006f, 0.002f);
	glVertex3f(-0.05f, 0.006f, 0.002f);
	glVertex3f(-0.05f, -0.006f, 0.002f);
	glVertex3f(-0.01f, -0.006f, 0.002f);
	//1---------------------------
	glNormal3f(0.0f, 0.0f, -1.0f);
...
void Display(void)
{
    ...
    glClear(GL_DEPTH_BUFFER_BIT | GL_COLOR_BUFFER_BIT);
   	...
   	GLfloat light0_pos[] = {-1.0, 0.0, 0.0, 1.0};
   ...
   glPushMatrix();
   glRotatef(30, 0.0, 0.0, 1.0);
   glRotatef(satelliter_Angle, 0.0, 1.0, 0.0);                         //orbit-rotate of satellite
   glTranslatef(0.0, 0.0, SATELLITE_ORBIT / 10 + 0.3*EARTH_RADIUS);

   glPushMatrix();
   glRotatef(45, 0.0, 0.0, 1.0);                        //PRECESSION of satellite
   glRotatef(satellites_Angle, 0.0, 1.0, 0.0);          //self-rotate of satellite
   Satellite();											//satellite
   glPopMatrix();
   glPopMatrix();

   glPushMatrix();
   glRotatef(PRECESSION_ANGLE, 0.0, 0.0, 1.0);                    //PRECESSION of earth
   glRotatef(earthaxis_Angle, 0.0, 1.0, 0.0);                     //self-rotate of earthaxis
   Earthaxis();
   glPopMatrix();
   ...
   glPushMatrix();
   glRotatef(neptuner_Angle, 0.0, 1.0, 0.0);                           //orbit-rotate of venus  
   glTranslatef(0.0, 0.0, NEPTUNE_ORBIT/10);
   glPushMatrix();
   glRotatef(neptunes_Angle, 0.0, 1.0, 0.0);                           //self-rotate of venus
   Venus();
   Neptune();
   glPopMatrix();   
   glPopMatrix();
...
void Idle(void)
{	   
   //Force the system to display after the event
}
...
void main(int argc, char** argv)
{
   ...
   glutCreateWindow("Project: Solor System and satellites");
